...
class RNNModel:
    def _init_(self, args, textdata):
        ...
        self.args = args               #TODO:- Remove dependency of args
    ...
    def build_network(self):
        ...
        enc_dec_cell = tf.contrib.rnn.BasicLSTMCell(self.args.hiddenSize,
                                                    state_is_tuple=True)
        if not self.args.test:  # TODO: Should use a placeholder instead
            enc_dec_cell = tf.contrib.rnn.DropoutWrapper(enc_dec_cell,
                                                         input_keep_prob=1.0,
                                                         output_keep_prob=0.5)
        enc_dec_cell = tf.contrib.rnn.MultiRNNCell([enc_dec_cell] * self.args.numLayers,
                                                   state_is_tuple=True)
        with tf.name_scope('placeholder_encoder'):
            self.encoder  = [tf.placeholder(tf.int32, [None, ]) for _ in range(self.args.maxLenEnco)]

        with tf.name_scope('placeholder_decoder'):
            #TODO:- check if operation name is not necessary for placeholders
            self.decoder  = [tf.placeholder(tf.int32, [None, ]) for _ in range(self.args.maxLenDeco)]
            self.decoder_weights=[tf.placeholder(tf.float32,[None,]) for _ in range(self.args.maxLenDeco)];
            self.decoder_target  = [tf.placeholder(tf.int32, [None, ]) for _ in range(self.args.maxLenDeco)]
